 ...
 */
export function personKey(email, pepper) {
    if (isBrowser) {
        throw new IdentityError('personKey is server only. Computing it in a browser lets anyone enumerate keys for any email.');
    }
    if (typeof pepper !== 'string' || pepper.length < MIN_PEPPER_LENGTH) {
        throw new IdentityError(`pepper must be a string of at least ${MIN_PEPPER_LENGTH} characters. Got ${typeof pepper === 'string' ? `${pepper.length} characters` : typeof pepper}.`);
    }
 ...
 */
export function pepperFromEnv(varName = 'BREAZY_IDENTITY_PEPPER') {
    const value = process.env[varName];
    if (!value) {
        throw new IdentityError(`${varName} is not set. Cross product identity cannot be derived without it, and falling back to a default would make every environment's keys collide.`);
    }
    if (value.length < MIN_PEPPER_LENGTH) {
        throw new IdentityError(`${varName} is shorter than ${MIN_PEPPER_LENGTH} characters.`);
    }
    return value;
}
/**
 ...
 */
export function personKeyEquals(a, b) {
